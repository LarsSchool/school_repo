...
void readAndSendSoftSerial() {
  ...
  if (idReader.available() > 0 &&
      writeOrRead) { // check eerst of er een id is verzonden en dat je ook aan
                     // het lezen state zit.
    byte receivedData = idReader.read();
    id = receivedData % 10;
    machineId = (receivedData - id) / 10;
    if (id < 10 && machineId < 25) // De id klopt stuur een goed response en
                                   // zorg ervoor dat data begint met lezen.
    {
      dataReader.listen();
      idReader.write(1);
      writeOrRead = false;
    ...
    } else // Gaat niet goed stuur een 2 om aan te geven dat het niet is gelukt
           // en luister weer naar een id.
    {
      ...
      writeOrRead = true;
             ...
             RESPONSETIME) // kijk of het antwoord binnen 50ms binnen komt,
                           // anders stuur error response.
  {
