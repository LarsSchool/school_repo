...
const byte GATEPINS[NROFGATES] = {9, 8};
const byte POTENTIOPIN = A2;
gatesRunTestStatus_t gatesRunTestStatus;
country_t country = NL;
...
const byte TESTLIGHTINTERVAL = 250;
const int BLINKINTERVAL = 500;
unsigned long previousGateMillis = 0;

signed int gatePosition[NROFGATES];
...
void toggleGatesTest() {
  gatesRunTestStatus = (gatesRunTestStatus == GATETESTING) ? GATERUNNING : GATETESTING;
  for (byte gate = 0; gate < NROFGATES; gate++) {
